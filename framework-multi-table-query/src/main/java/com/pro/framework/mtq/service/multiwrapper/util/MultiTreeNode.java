...
public class MultiTreeNode<T> {
    ...
    public <R> List<R> readNodes(Function<MultiTreeNode<T>, R> nodeReadFun) {
        ArrayList<R> list = new ArrayList<>(1024);
        list.add(nodeReadFun.apply(this));
        if (this.children != null) {
            for (MultiTreeNode<T> child : this.children) {
                list.addAll(child.readNodes(nodeReadFun));
            }
        }
        return list;
    }
    ...
    public void consumerTopToBottom(Consumer<T> consumer) {
        if (parent != null) {
            log.warn("不是从树根节点开始执行topToBottom的:" + JSONUtils.toString(parent));
        }
